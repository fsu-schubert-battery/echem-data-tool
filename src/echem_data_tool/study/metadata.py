...
"""
Metadata classes for the hierarchical study structure.

This module defines metadata containers following the Battery Data Genome (BDG)
specification for all hierarchy levels:
- StudyMetadata: Study-level (global) metadata with contributors and funding
- CellMetadata: Cell-level metadata with chemistry, components, and assembly info
- TechniqueMetadata: Technique-level metadata with devices and settings
- AuxiliaryMetadata: Auxiliary data metadata with devices and parent techniques
- GroupMetadata: Group-level metadata for organizational units

Additionally provides helper classes for structured metadata:
- Contributor, Funding: Study-level helpers 
- Material, Component, ChemistryObject, AssemblyObject: Cell-level helpers
- Device, Setting, AdditionalNote: Technique/Auxiliary-level helpers 
- AmountObject: General quantity representation

All metadata classes support conversion to/from netCDF attributes and JSON
for file I/O and data exchange.
"""
...
# GENERAL METADATA
# =============================================================================
# Base classes and general helper classes used across all metadata levels
...
# =============================================================================
# Metadata classes and helpers for study-level information
...
# CELL-LEVEL METADATA
# =============================================================================
# Metadata classes and helpers for cell-level information
...
class Component:
    ...
    procedures: List[Setting] = field(default_factory=list)
    properties: List[Setting] = field(default_factory=list)
    ...
    def add_procedure(self, name: str, value: Union[str, int, float, List[Any]], 
                     unit: Optional[str] = None) -> Setting:
        """Add a procedure setting."""
        procedure = Setting(name=name, value=value, unit=unit)
    ...
    def add_property(self, name: str, value: Union[str, int, float, List[Any]], 
        ...
        property_param = Setting(name=name, value=value, unit=unit)
    ...
    def find_procedure(self, name: str) -> Optional[Setting]:
    ...
    def find_property(self, name: str) -> Optional[Setting]:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> Component:
        ...
        for proc_data in data.get("procedures", []):
            component.procedures.append(Setting.from_dict(proc_data))
        
        # Reconstruct properties
        for prop_data in data.get("properties", []):
            component.properties.append(Setting.from_dict(prop_data))
...
class CellMetadata(BaseMetadata):
    ...
    def add_component(self, name: str, materials: Optional[List[tuple]] = None,
                     ...
                     properties: Optional[List[tuple]] = None) -> Component:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> CellMetadata:
        ...
        for comp_data in secondary.get("components", []):
            cell.components.append(Component.from_dict(comp_data))
    ...
    def from_netcdf_attrs(cls, attrs: Dict[str, Any]) -> CellMetadata:
        ...
        if "cell_metadata_json" in attrs:
            ...
            secondary = data.get("secondary", {})
            for comp_data in secondary.get("components", []):
                component = Component(name=comp_data["name"])
...
# =============================================================================
# TECHNIQUE & AUXILIARY-LEVEL METADATA
# =============================================================================
# Metadata classes and helpers for technique and auxiliary measurements
...
class Device:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.software_name is not None or self.software_version is not None:
            result["software"] = {
    ...
    def from_dict(cls, data: Dict[str, Any]) -> Device:
        ...
        if "software" in data:
            software_name = data["software"].get("name")
...
@dataclass
class Setting:
    """Represents a measurement setting or property.
    
    Used for:
    - Measurement settings in techniques/auxiliaries (e.g., scan_rate, potential_range)
    - Component procedures in cell assembly (e.g., drying_temperature, coating_method)
    - Component properties (e.g., diameter, mass, thickness)
    
    Attributes:
        name: Setting name
        value: Setting value (can be numeric, string, or list)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> Setting:
        """Create Setting from dictionary representation."""
        return cls(
            name=data["name"],
            value=data["value"],
            unit=data.get("unit")
...
class TechniqueMetadata(BaseMetadata):
    ...
    def add_setting(self, name: str, value: Union[str, int, float, List[Any]], 
        ...
        setting = Setting(name=name, value=value, unit=unit)
    ...
    def find_setting(self, name: str) -> Optional[Setting]:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> TechniqueMetadata:
        ...
        for setting_data in secondary.get("settings", []):
            technique.settings.append(Setting.from_dict(setting_data))
...
class AuxiliaryMetadata(BaseMetadata):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> AuxiliaryMetadata:
        ...
        # Reconstruct settings
        for setting_data in secondary.get("settings", []):
            auxiliary.settings.append(Setting.from_dict(setting_data))
